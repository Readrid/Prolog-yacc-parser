...
def test_tree_atom_scopes(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.chdir(tmp_path)
    main.main(['a.txt'])
    assert open('a.txt.out', 'r').read() == "REL (ATOMEXPR (ATOMBODY (ID F) \
                  (ATOM ((ATOMBODY (ID cons) (ATOM (ATOMEXPR (ID H)))) \
                   ATOM (ATOMEXPR (ID p)))))) (ID f)\n"
...
def test_tree_atom_concat(tmp_path, monkeypatch, capsys):
    ...
    main.main(['a.txt'])
    assert open('a.txt.out', 'r').read() == 'REL (ATOMEXPR (ATOMBODY (ID F) \
                   (ATOM (ATOMEXPR (ATOMBODY (ID g) \
                   (ATOM (ATOMEXPR (ID t)))))))) (ID f)\n'
...
def test_incorrect_without_dot(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.chdir(tmp_path)
    main.main(['a.txt'])
    assert open('a.txt.out', 'r').read() == 'Syntax error: line 2, colon WTF\n'
...
def test_incorrect_without_dot_one_line(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.chdir(tmp_path)
    main.main(['a.txt'])
    assert open('a.txt.out', 'r').read() == 'Syntax error: line 1, colon WTF\n'

def test_incorrect_head(tmp_path, monkeypatch, capsys):
    (tmp_path / 'a.txt').write_text('f.\n dwwd :- wef. \n :- f ; e')
    monkeypatch.chdir(tmp_path)
    main.main(['a.txt'])
    assert open('a.txt.out', 'r').read() == 'Syntax error: line 3, colon WTF\n'
